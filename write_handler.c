	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ')/* Asign extra char to left of buffer */
		{
			if (extra_c)
				buffer[--i] = extra_c;
			return (write(1, &buffer[i], length) + write(1, &buffer[1], k - 1));
		}
		else if (!(flags & F_MINUS) && padd == ' ')/* extra char to left of buff */
		{
			if (extra_c)
				buffer[--i] = extra_c;
			return (write(1, &buffer[1], k - 1) + write(1, &buffer[i], length));
		}
...
/**
 * write_unsned - Writes an unsigned number
 ...
 */
int write_unsned(int is_negative, int ind,
	char buffer[],
	int flags, int width, int precision, int size)
{
